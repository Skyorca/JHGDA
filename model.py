...
class GCNPooling(nn.Module):
    ...
    def forward(self,X_old, edge_index, edge_weight, A_old, Y_old, Z, use_sparse=False):
        ...
        # stat = {}
        # for v in range(0,11):
        #     stat[v*0.1] = torch.sum(A_new>=(v*0.1)).item()-torch.sum(A_new>((v+1)*0.1)).item()
        # print(stat)
        print('sparse ratio: ', torch.sum(A_new==0)/A_new.shape[0]**2)
    ...
    def __init__(self, conv_params, pool_params_s, pool_params_t,n_class,device, drop_p=0, classwise=False, share=0, sparse = 0):
        ...
        if not share:
            ...
            for i in range(self.num_pool_blocks):
                self.gnn_pool_src.append(GCNPooling(pool_params_s[i],drop_p, device=device))
                self.gnn_pool_tgt.append(GCNPooling(pool_params_t[i],drop_p, device=device))
            self.gnn_pool_src = nn.ModuleList(self.gnn_pool_src)
            self.gnn_pool_tgt = nn.ModuleList(self.gnn_pool_tgt)
        else:
            self.gnn_pool = []
            for i in range(self.num_pool_blocks):
                self.gnn_pool.append(GCNPooling(pool_params_s[i], device=device))
    ...
    def forward(self, x_s, edge_index_s, y_s, x_t, edge_index_t, y_t):
        ...
        for i in range(self.num_conv_blocks):
            ...
            if i<self.num_pool_blocks:
                ...
                print(f"source {edge_index_s.shape[1]} edges, target {edge_index_t.shape[1]} edges")
...
class SingleCondDiffPool(nn.Module):
    def __init__(self, conv_params, pool_params):
        """

        :param conv_params: [[1st conv layer 参数],[],...]
        :param pool_params: [[1st pool layer 参数],[],...]
        :param pool_ratio: [r1,r2]
        :param max_num_nodes: 输入原始图的节点数
        :param n_class: 类别数，最终输出的cluster数
        """
        super(SingleCondDiffPool, self).__init__()
        self.num_conv_blocks = len(conv_params)
        self.num_pool_blocks = len(pool_params)
        assert self.num_conv_blocks >= self.num_pool_blocks
        # 组装网络
        self.gnn_emb = []  # embedding gnn is shared between domains
        for i in range(self.num_conv_blocks):
            self.gnn_emb.append(GCNEncoder(conv_params[i]))
        self.gnn_pool = []  # pooling gnn is seperated between domains
        for i in range(self.num_pool_blocks):
            self.gnn_pool.append(GCNPooling(pool_params[i]))
            print(pool_params[i])

    def adj2coo(self, A):
        # note
        edge_weight = torch.squeeze(A.reshape(1,-1))  # shape is (dim0,) instead of (1,dim0)
        row_elements = []
        col_elements = []
        n_node = A.shape[0]
        for i in range(n_node):
    ...
    def forward(self, x_s, edge_index_s, y_s):
        edge_weight_s = torch.ones(edge_index_s.shape[1])
        A_s= None  # 第一层pooling不需要原始图的邻接矩阵表示
        for i in range(self.num_conv_blocks):
            print(f"@conv block {i+1}")
            # message passing
            z_s = self.gnn_emb[i](x_s, edge_index_s, edge_weight_s)
            ...
            if i<self.num_pool_blocks:
                print(f"@pool block {i + 1}")
                use_sparse = True if i==0 else False
                S_s, x_s, A_s, y_s = self.gnn_pool[i](X_old=x_s, edge_index=edge_index_s, edge_weight=edge_weight_s, A_old=A_s, Y_old=y_s, Z=z_s, use_sparse=use_sparse)
                edge_index_s, edge_weight_s = self.adj2coo(A_s)
                print(S_s.shape)
        return x_s
